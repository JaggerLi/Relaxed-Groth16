...
func (primary *CommitedRelaxedR1CS) FoldProve(r1cs *cs.R1CS, secondary *CommitedRelaxedR1CS, pk_e PedersenKey) (*CommitedRelaxedR1CS, bn254.G1Affine, error) {
	...
	az1, az2, bz1, bz2, cz1, cz2 := parallelMatrixMulFast(r1cs, z1, z2)
	t1 := batchAdd(batchMul(az1, bz2), batchMul(az2, bz1))
	t2 := batchAdd(scalarMul(primary.U, cz2), scalarMul(secondary.U, cz1))
	t := batchSub(t1, t2)
	// fmt.Println("FoldProve Stage 1: ", time.Since(start_time))
	// Commit T
	...
	// fmt.Println("FoldProve Stage 2: ", time.Since(start_time))

	//Sample random r
	com_t_bytes := com_t.RawBytes()
	r_val := mimcHash(com_t_bytes[:])
	// fmt.Println("FoldProve Stage 3: ", time.Since(start_time))
	...
	// fmt.Println("FoldProve Stage 4: ", time.Since(start_time))
...
// parallelMatrixMulFast partitions the constraints among workers (goroutines)
// and computes all six accumulations in each worker.
func parallelMatrixMulFast(r1cs *cs.R1CS, z1, z2 []fr.Element) ([]fr.Element, []fr.Element, []fr.Element, []fr.Element, []fr.Element, []fr.Element) {
	n := len(r1cs.Constraints)
	az1 := make([]fr.Element, n)
	az2 := make([]fr.Element, n)
	bz1 := make([]fr.Element, n)
	bz2 := make([]fr.Element, n)
	cz1 := make([]fr.Element, n)
	cz2 := make([]fr.Element, n)

	numWorkers := runtime.NumCPU() // or set to a tuned value
	chunkSize := (n + numWorkers - 1) / numWorkers

	var wg sync.WaitGroup
	wg.Add(numWorkers)
	for w := 0; w < numWorkers; w++ {
		// Capture the worker's index range.
		start := w * chunkSize
		end := start + chunkSize
		if end > n {
			end = n
		...
		go func(start, end int) {
			defer wg.Done()
			// Process each constraint in the worker's assigned range.
			for i := start; i < end; i++ {
				c := r1cs.Constraints[i]
				fusedAz1, fusedAz2 := accumulateLinearExpFused(r1cs, c.L, z1, z2)
				az1[i] = fusedAz1
				az2[i] = fusedAz2
				fusedBz1, fusedBz2 := accumulateLinearExpFused(r1cs, c.R, z1, z2)
				bz1[i] = fusedBz1
				bz2[i] = fusedBz2
				fusedCz1, fusedCz2 := accumulateLinearExpFused(r1cs, c.O, z1, z2)
				cz1[i] = fusedCz1
				cz2[i] = fusedCz2
				// az1[i] = accumulateLinearExp(r1cs, c.L, z1)
				// az2[i] = accumulateLinearExp(r1cs, c.L, z2)
...
func accumulateLinearExpFused(r1cs *cs.R1CS, l constraint.LinearExpression, z1, z2 []fr.Element) (fr.Element, fr.Element) {
	var r1, r2 fr.Element
	r1.SetZero()
	r2.SetZero()
	for _, t := range l {
		cID := t.CoeffID()
		// If constant, add the fixed coefficient to both accumulators.
		if t.IsConstant() {
			r1.Add(&r1, &r1cs.Coefficients[cID])
			r2.Add(&r2, &r1cs.Coefficients[cID])
			continue
		}
		vID := t.WireID()
		// Compute the contribution from this term for both z1 and z2.
		switch cID {
		case constraint.CoeffIdZero:
			continue
		case constraint.CoeffIdOne:
		...
		default:
			var t1, t2 fr.Element
			t1.Mul(&r1cs.Coefficients[cID], &z1[vID])
			t2.Mul(&r1cs.Coefficients[cID], &z2[vID])
			r1.Add(&r1, &t1)
			r2.Add(&r2, &t2)
		}
	}
