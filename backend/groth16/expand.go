...
func (circuit *ExpandCircuit) Define(api frontend.API) error {
	...
	hasher1, err := NewHasher(api)
	if err != nil {
		return err
	}
	hasher1.Write(frontend.Variable(1))
	hasher1.Write(circuit.Z0...)
	// Pre-compute the value of Z1
	// z1 := block(api, circuit.Z0, circuit.Wi)
	z1 := make([]frontend.Variable, len(circuit.Z0))
	if len(z1) != len(Param.PreHash) {
		return errors.New("invalid length of Z0")
	}
	for i := 0; i < len(z1); i++ {
		z1[i] = Param.PreHash[i]
	}
	hasher1.Write(z1...)
	// M=N=0, L=3
	...
	ret2 := hasher2.Sum()
	isValid1 := IsEqual(api, circuit.SmallInstance.X[0], ret2)
	comEJac = bn254.G1Jac{}
	comEJac.FromAffine(&emptyIns.Com_E)
	isValid21 := IsEqual(api, circuit.SmallInstance.Com_E.X, frontend.Variable(comEJac.X))
	isValid22 := IsEqual(api, circuit.SmallInstance.Com_E.Y, frontend.Variable(comEJac.Y))
	isValid23 := IsEqual(api, circuit.SmallInstance.Com_E.Z, frontend.Variable(comEJac.Z))
	...
	isValid5 := IsEqual(api, circuit.U, frontend.Variable(1))
	api.AssertIsDifferent(api.Add(isValid1, isValid21, isValid22, isValid23, isValid3, isValid4, isValid5), 7)
	return nil
}
