...
func Test_ExpandCircuit(t *testing.T) {
	...
	pk_e, pk_w, err := IVCSetup(_r1cs)
	internal, secret, public := _r1cs.GetNbVariables()
	bigInstance, err := NewCommittedRelaxedR1CS(len(_r1cs.Coefficients), internal+secret+public, public, pk_e, pk_w)
	// Make assignment
	if err != nil {
		panic(err)
	}
	comT := bigInstance.Com_E
	assignment, _, err := MakeAssignment(bigInstance, bigInstance, comT, 0, initialState[:], initialState[:], message[:64])
	if err != nil {
		panic(err)
	}
	assert.ProverSucceeded(&circuit, &ExpandCircuit{
