...
func IVCSetup(r1cs *cs.R1CS) (pk_e, pk_w PedersenKey, err error) {
	...
	//if pk_w, err = PedersenSetup(r1cs.GetNbInternalVariables() + r1cs.GetNbPublicVariables() + r1cs.GetNbSecretVariables() - 4); err != nil {
	if pk_w, err = PedersenSetup(r1cs.GetNbInternalVariables() + r1cs.GetNbSecretVariables()); err != nil {
		return
	}
	return
}
...
func IVCProve(r1cs *cs.R1CS, pk_e, pk_w PedersenKey, inputs []byte) (*CommitedRelaxedR1CS, bn254.G1Affine, error) {
	...
	bigInstance, err := NewCommittedRelaxedR1CS(r1cs.GetNbConstraints(), r1cs.GetNbInternalVariables()+r1cs.GetNbPublicVariables()+r1cs.GetNbSecretVariables(), public-1, pk_e, pk_w)
	// fmt.Println("Stage 1: ", time.Since(startTime))
	...
	// fmt.Println("Stage 2: ", time.Since(startTime))
	for i := 1; i < num_iter; i++ {
		if bigInstance, comT, err = bigInstance.FoldProve(r1cs, smallInstance, pk_e); err != nil {
			return nil, bn254.G1Affine{}, err
		}
		// fmt.Println("Stage 3 Prove, Iter ", i, ": ", time.Since(startTime))
		...
		// fmt.Println("Stage 3 End, Iter ", i, ": ", time.Since(startTime))
	}
	bigInstance, comT, err = bigInstance.FoldProve(r1cs, smallInstance, pk_e)
	if err != nil {
		return nil, bn254.G1Affine{}, err
	}
	// fmt.Println("Stage 4: ", time.Since(startTime))
...
func MakeAssignment(bigInstance, smallInstance *CommitedRelaxedR1CS, comT bn254.G1Affine, idx int, z0, zi []uint32, wi []byte) (*ExpandCircuit, []uint32, error) {
	...
	if idx == 0 {
		// Param.PreHash = make([]uint32, len(new_z))
		copy(Param.PreHash[:], new_z)
	}
	for _, z := range new_z {
		hashInputs = append(hashInputs, u32ToByteArray(z))
